...
class SleepQualityViewModel(
    ...
    init {
        _navigateBackToSleepTracker.value = false
    }
    ...
    fun onSetSleepQuality(qualityRating: Int) {
    ...
    private suspend fun setQuality(quality: Int) {
        return withContext(Dispatchers.IO) {
            val nightToRate = database.get(sleepNightKey) ?: return@withContext
            nightToRate.sleepQuality = quality
            database.update(nightToRate)
        }
    }
}
